...
export class SignupPage {
  ...
  setProfile(user: any){
    this.profiles = this.af.list('/profiles' );

    return this.profiles.update(user.uid, { 
      email: user.email,
  ...
  doSignup(){
    ...
    this.userService.registerUser(this.signup.value).then((user) => {
      ...
      .catch(err => {
        ...
        let alert = env.alertCtrl.create({
            ...
            subTitle: 'Please make sure all fields are filled in and your password is strong enough.',
            buttons: ['OK']
          });
          alert.present();
